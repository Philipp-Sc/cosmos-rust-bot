...
pub async fn lazy_anchor_account_auto_farm_rewards(maybes: &HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, is_test: bool) -> Vec<(Entry, Pin<Box<dyn Future<Output=Maybe<String>> + Send + 'static>>)> {
    ...
    let t2: Pin<Box<dyn Future<Output=Maybe<String>> + Send + 'static>> = Box::pin(simulation_swap_exchange_rate_to_string(maybes.clone(), "swap_simulation,terraswap,Anchor,ANC,none,uusd", false, 2));
    view.push((t1, t2));
