...
// replace group: return maybe_struct!((Some($2),Some(Utc::now().timestamp())));
...
pub async fn calculate_borrow_plan(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field: &str, digits_rounded_to: u32) -> Maybe<String> {

    let mut ust_amount_liquid = decimal_or_return!(terra_balance_to_string(tasks.clone(),"uusd",false,10).await);
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"min_ust_balance",false,10).await);
    ...
    let borrow_amount = decimal_or_return!(calculate_amount(tasks.clone(),"borrow",false,10).await);

    let uusd_tax_cap = decimal_or_return!(uusd_tax_cap_to_string(tasks.clone(),false,10).await);
    let tax_rate = decimal_or_return!(tax_rate_to_string(tasks.clone(),10).await);
    ...
    let anchor_protocol_txs_borrow_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_borrow_stable","avg_gas_used".to_owned(),false,10).await);
    ...
    let anchor_protocol_txs_deposit_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_deposit_stable","avg_gas_used".to_owned(),false,10).await);
...
pub async fn calculate_repay_plan(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field: &str, digits_rounded_to: u32) -> Maybe<String> {

    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"min_ust_balance",false,10).await);
    
    let ust_amount_liquid = decimal_or_return!(terra_balance_to_string(tasks.clone(),"uusd",false,10).await)
                            .checked_sub(min_ust_balance).unwrap();  

    let ust_balance_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"ust_balance_preference",false,10).await);
    ...
    let repay_amount = decimal_or_return!(calculate_amount(tasks.clone(),"repay",false,10).await);
    ...
    let a_ust_deposit_liquid = decimal_or_return!(borrower_ust_deposited_to_string(tasks.clone(),false,10).await);
    ...
    let fee_to_redeem_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_redeem_stable","avg_gas_used".to_owned(),false,10).await);
    ...
    let anchor_protocol_txs_repay_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_repay_stable","avg_gas_used".to_owned(),false,10).await);
...
pub async fn calculate_farm_plan(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field: &str, as_micro: bool, digits_rounded_to: u32) -> Maybe<String> {
    ...
    let fee_to_claim_anc_rewards_uusd_in_anc = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","avg_gas_used".to_owned(),true,0).await)
            .checked_mul(gas_fees_uusd).unwrap()
            .checked_mul(gas_adjustment_preference).unwrap()
            .checked_mul(exchange_rate).unwrap();


    let fee_to_provide_in_anc = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "txs_provide_to_spec_anc_ust_vault","avg_gas_used".to_owned(),true,0).await)
            .checked_mul(gas_fees_uusd).unwrap()
            .checked_mul(gas_adjustment_preference).unwrap()
            .checked_mul(exchange_rate).unwrap();
    ...
    let borrow_anc_rewards = decimal_or_return!(borrower_rewards_to_string(tasks.clone(),true,0).await);
    ...
    let exchange_return = decimal_or_return!(simulation_swap_return_amount_to_string(tasks.clone(),"simulation_cw20 anchorprotocol ANC terraswapAncUstPair",false,10).await);
...
pub async fn calculate_amount(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, key: &str, as_micro: bool, digits_rounded_to: u32) -> Maybe<String> {
    ...
    let target_percent = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"target_percentage",false,10).await); 
    let zero =  Decimal::from_str("0").unwrap(); 
...
pub async fn check_anchor_loan_status(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>,key: &str, digits_rounded_to: u32) -> Maybe<String> {
 
    let zero =  Decimal::from_str("0").unwrap(); 

    let trigger_percentage = match key {
        "repay" => {decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await)},
        "borrow" => {decimal_or_return!(meta_data_key_to_string(tasks.clone(),"borrow_percentage",false,10).await)},
...
pub async fn estimate_anchor_protocol_tx_fee_claim_and_farm(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
  ...
  let tx_fee_claim_rewards = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","fee_amount_at_threshold".to_owned(),false,10).await);   
  let tx_fee_provide_liquidity = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_provide_liquidity","fee_amount_at_threshold".to_owned(),false,10).await);   
  let tx_fee_stake_rewards = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_staking_lp","fee_amount_at_threshold".to_owned(),false,10).await);   
...
pub async fn estimate_spec_tx_fee_provide(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    let gas_fees_uusd = decimal_or_return!(gas_price_to_string(tasks.clone(),10).await);

    let gas_adjustment_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"gas_adjustment_preference",false,10).await);

    let tx_fee_claim_rewards_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "txs_provide_to_spec_anc_ust_vault","avg_gas_used".to_owned(),false,10).await);   
...
pub async fn estimate_anchor_protocol_tx_fee_claim(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    let gas_fees_uusd = decimal_or_return!(gas_price_to_string(tasks.clone(),10).await);

    let gas_adjustment_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"gas_adjustment_preference",false,10).await);

    let tx_fee_claim_rewards_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","avg_gas_used".to_owned(),false,10).await);   
...
pub async fn estimate_anchor_protocol_tx_fee_claim_and_provide_to_spec_vault(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
 
    let gas_fees_uusd = decimal_or_return!(gas_price_to_string(tasks.clone(),10).await);
    let gas_adjustment_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"gas_adjustment_preference",false,10).await);

    let fee_to_claim_anc_rewards_uusd = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","avg_gas_used".to_owned(),false,10).await);
    let fee_to_provide = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "txs_provide_to_spec_anc_ust_vault","avg_gas_used".to_owned(),false,10).await);
...
pub async fn estimate_anchor_protocol_tx_fee_claim_and_stake(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    let gas_fees_uusd = decimal_or_return!(gas_price_to_string(tasks.clone(),10).await);

    let gas_adjustment_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"gas_adjustment_preference",false,10).await);

    let tx_fee_claim_rewards_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","avg_gas_used".to_owned(),false,10).await);   
    let tx_fee_stake_rewards_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_staking","avg_gas_used".to_owned(),false,10).await);   
...
pub async fn estimate_anchor_protocol_tx_fee(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, tx_key: &str, key: String, as_micro: bool, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn apy_on_collateral_by(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, amount_field: &str, apr_field: &str, digits_rounded_to: u32) -> Maybe<String> { 
    ...
    if amount_field == "loan_amount" {
        ...
        loan_amount = decimal_or_return!(borrower_ust_deposited_to_string(tasks.clone(),false, 10).await); 

    }else if amount_field == "target_ltv" { 
        loan_amount = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await)
    ...
    if "net_apr" == apr_field { 
        let net_apr = percent_decimal_or_return!(net_apr_to_string(tasks.clone(),  10).await); 
        let earn_apr = percent_decimal_or_return!(earn_apr_to_string(tasks.clone(),  10).await); 
        apr = net_apr.checked_add(earn_apr).unwrap();
    }else if "earn_apr" == apr_field { 
        apr = percent_decimal_or_return!(earn_apr_to_string(tasks.clone(),  10).await);
    }else if "borrow_apr"== apr_field {
        apr = percent_decimal_or_return!(borrow_apr_to_string(tasks.clone(),  10).await);
    }else if "distribution_apr" == apr_field {
        apr = percent_decimal_or_return!(distribution_apr_to_string(tasks.clone(),  10).await);
    }
...
pub async fn anc_staked_balance_in_ust_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn anchor_claim_and_stake_transaction_gas_fees_ratio_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    ...
    let anchor_protocol_tx_fee = decimal_or_return!(estimate_anchor_protocol_tx_fee_claim_and_stake(tasks.clone(),  10).await);    
...
pub async fn borrower_rewards_in_ust_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    let mut _pending_rewards = Decimal::from_str("0").unwrap();
    match get_data_maybe_or_await_task(&tasks,"borrow_info").await {
        Ok(response_result) => { 
            _pending_rewards = Decimal::from_str(response_result.as_borrow_info().unwrap().result.pending_rewards.to_string().as_str()).unwrap();
...
pub async fn borrower_deposit_liquidity_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn borrower_ltv_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn borrower_ust_deposited_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, as_micro: bool, digits_rounded_to: u32) -> Maybe<String> { 
    let mut _balance = Decimal::from_str("0").unwrap();
    match get_data_maybe_or_await_task(&tasks,"balance").await {
        Ok(response_result) => { 
            _balance = Decimal::from_str(response_result.as_balance().unwrap().result.balance.to_string().as_str()).unwrap();
...
pub async fn borrow_apr_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn anchor_airdrops_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>) -> Maybe<String> {  
...
pub async fn anything_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, key: &str) -> Maybe<String> { 
...
pub async fn net_apr_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
        // utilisationRatio = stablecoinsLent / stablecoinsDeposited
        // borrowRate = utilisationRatio * interestMultiplier + baseRate
        // borrow_apr = blocksPerYear * borrowRate
...
pub async fn borrow_rate_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, key: &str, key_1: &str, key_2: &str, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn utilization_ratio_to_string(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
...
pub async fn estimate_anchor_protocol_auto_repay_tx_fee(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    ...
    let stability_tax = decimal_or_return!(calculate_repay_plan(tasks.clone(),"stability_tax",10).await);
...
pub async fn estimate_anchor_protocol_auto_borrow_tx_fee(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, digits_rounded_to: u32) -> Maybe<String> { 
    ...
    let anchor_protocol_txs_borrow_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_borrow_stable","avg_gas_used".to_owned(),false,10).await);
    // min(to_repay * tax_rate , tax_cap)
    let stability_tax_borrow = decimal_or_return!(calculate_borrow_plan(tasks.clone(),"stability_tax_borrow",10).await);

    let anchor_protocol_txs_deposit_stable_gas_used = decimal_or_return!(estimate_anchor_protocol_tx_fee(tasks.clone(),"anchor_protocol_txs_deposit_stable","avg_gas_used".to_owned(),false,10).await);
    // min(to_repay * tax_rate , tax_cap) 
    let stability_tax_deposit = decimal_or_return!(calculate_borrow_plan(tasks.clone(),"stability_tax_deposit",10).await);
