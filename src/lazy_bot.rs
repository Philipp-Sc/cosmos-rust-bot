...
pub async fn lazy_anchor_account_auto_farm_rewards(tasks: &Arc<RwLock<HashMap<String, MaybeOrPromise>>>, wallet_acc_address: &Arc<SecUtf8>, wallet_seed_phrase: &Arc<SecUtf8>,  new_display: &Arc<RwLock<Vec<String>>>,offset: &mut usize, is_test: bool, is_first_run: bool) -> Vec<(usize,Pin<Box<dyn Future<Output = String> + Send + 'static>>)> {
    ...
    *offset += 1;
    ...
    // use estimate here to predict the fee.


    // TX FEE Claim && SWAP 50% + Fee TO UST 
    // ANC inside Pool
    // UST inside Pool
    // https://finder.terra.money/mainnet/address/terra1qr2k6yjjd5p2kaewqvg93ag74k6gyjr7re37fs
    // estimate recieved LP tokens
    // - 0.01% slippage
    // then provide to stake in one transactions.
    // TX FEE provide liquidity 
    // TX FEE stake lp



    anchor_view.push((format!("{}{}","\n\n   [Auto Farm UST]".truecolor(75,219,75),"         balance:           ".purple().to_string()),*offset));
    *offset += 1;
    ...
    *offset += 1;
    ...
    anchor_view.push((format!("{}{}","\n\n   [Auto Farm ANC-UST LP]".truecolor(75,219,75),"         balance:           ".purple().to_string()),*offset));
    *offset += 1;
    ...
    *offset += 1;
    ...
    anchor_view.push((" LP".purple().to_string(),*offset));
    *offset += 1;
 
    anchor_view.push((format!("{}{}","\n   [Auto Farm ANC-UST LP]".truecolor(75,219,75),"         staked:            ".purple().to_string()),*offset));
    *offset += 1;
    ...
    *offset += 1;
    ...
    anchor_view.push((format!("{}{}","\n   [Auto Farm Rewards]".truecolor(75,219,75),"     amount:            ".purple().to_string()),*offset));
    *offset += 1;
    ...
    *offset += 1;
    ...
    // est fees.
    anchor_view.push((format!("{}{}","\n\n   [Auto Farm Claim Tx]".truecolor(75,219,75),"    est. fee:          ".purple().to_string()),*offset));
    *offset += 1;

    anchor_view.push(("--".purple().to_string(),*offset));
    let t: (usize,Pin<Box<dyn Future<Output = String> + Send + 'static>>) = (*offset, Box::pin(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_claim_rewards","fee_amount_at_threshold".to_owned(),false,2)));
    anchor_tasks.push(t);
    *offset += 1;
    ...
    *offset += 1;
    ...
    // est fees.
    anchor_view.push((format!("{}{}","\n   [Auto Farm Provide Tx]".truecolor(75,219,75),"  est. fee:          ".purple().to_string()),*offset));
    *offset += 1;

    anchor_view.push(("--".purple().to_string(),*offset));
    let t: (usize,Pin<Box<dyn Future<Output = String> + Send + 'static>>) = (*offset, Box::pin(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_provide_liquidity","fee_amount_at_threshold".to_owned(),false,2)));
    anchor_tasks.push(t);
    *offset += 1;
    ...
    *offset += 1;
    ...
    // est fees.
    anchor_view.push((format!("{}{}","\n\n   [Auto Farm Stake Tx]".truecolor(75,219,75),"    est. fee:          ".purple().to_string()),*offset));
    *offset += 1;

    anchor_view.push(("--".purple().to_string(),*offset));
    let t: (usize,Pin<Box<dyn Future<Output = String> + Send + 'static>>) = (*offset, Box::pin(estimate_anchor_protocol_tx_fee(tasks.clone(), "anchor_protocol_txs_staking_lp","fee_amount_at_threshold".to_owned(),false,2)));
    anchor_tasks.push(t);
    *offset += 1;
    ...
    *offset += 1;
    ...
    anchor_view.push((format!("{}{}","\n   [Auto Farm Tx]".truecolor(75,219,75),"          est. fee:          ".purple().to_string()),*offset));
    *offset += 1;

    anchor_view.push(("--".purple().to_string(),*offset));
    let t: (usize,Pin<Box<dyn Future<Output = String> + Send + 'static>>) = (*offset, Box::pin(estimate_anchor_protocol_tx_fee_claim_and_farm(tasks.clone(),2)));
    anchor_tasks.push(t);
    *offset += 1;
