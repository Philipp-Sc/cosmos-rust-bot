...
pub async fn anchor_claim_and_stake_airdrops(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    ...
    match try_get_resolved(&maybes, "anchor_airdrops").await {
        Maybe { data: Ok(res), .. } => {
            ...
            match anchor_claim_and_stake_airdrop_tx(&asset_whitelist, &mnemonics, &vec_proof, &vec_stage, &vec_amount, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
                Ok(msg) => {
...
pub async fn anchor_borrow_and_deposit_stable(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    let balance = decimal_or_return!(terra_balance_to_string(maybes.clone(),"uusd",false,2).await);
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"min_ust_balance",false, 4).await);
    ...
    match anchor_borrow_and_deposit_stable_tx(&asset_whitelist, &mnemonics, to_borrow, to_deposit, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
        Ok(msg) => {
...
pub async fn anchor_redeem_and_repay_stable(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    let balance = decimal_or_return!(terra_balance_to_string(maybes.clone(),"uusd",false,2).await);
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"min_ust_balance",false, 4).await);
    ...
    if to_withdraw_from_deposit > zero && to_repay > zero {
        ...
        match anchor_redeem_and_repay_stable_tx(&asset_whitelist, &mnemonics, to_withdraw_from_deposit, to_repay, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
            Ok(msg) => {
        ...
        match anchor_repay_stable_tx(&asset_whitelist, &mnemonics, to_repay, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
            Ok(msg) => {
                register_value(&maybes, "anchor_redeem_and_repay_stable".to_string(), msg.to_owned()).await;
                register_value(&maybes, "latest_transaction".to_string(), msg.to_owned()).await;
                return maybe_struct!((Some( msg),Some(Utc::now().timestamp())));
...
pub async fn anchor_borrow_claim_and_farm_rewards(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    let balance = decimal_or_return!(terra_balance_to_string(maybes.clone(),"uusd",false,2).await);
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"min_ust_balance",false, 4).await);
    ...
    match anchor_governance_claim_and_provide_to_spec_vault(&asset_whitelist, &mnemonics, anc_to_keep, ust_to_keep, anc_to_swap, belief_price, max_spread, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
        Ok(msg) => {
...
pub async fn anchor_borrow_claim_and_stake_rewards(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    let balance = decimal_or_return!(terra_balance_to_string(maybes.clone(),"uusd",false,2).await);
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"min_ust_balance",false, 4).await);
    ...
    match anchor_governance_claim_and_stake(&asset_whitelist, &mnemonics, anc_to_claim, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
        Ok(msg) => {
...
pub async fn anchor_borrow_claim_rewards(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    ...
    match anchor_claim_rewards(&asset_whitelist, &mnemonics, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
        Ok(msg) => {
            return maybe_struct!((Some( msg),Some(Utc::now().timestamp())));
        }
        Err(msg) => {
...
pub async fn anchor_governance_stake_balance(asset_whitelist: Arc<AssetWhitelist>, maybes: HashMap<String, Arc<Mutex<Vec<Maybe<ResponseResult>>>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> Maybe<String> {
    let max_tx_fee = decimal_or_return!(meta_data_key_to_string(maybes.clone(),"max_tx_fee",false, 4).await);
    ...
    match anchor_governance_stake(&asset_whitelist, &mnemonics, anc_balance, gas_fees_uusd, max_tx_fee, gas_adjustment_preference, only_estimate).await {
        Ok(msg) => {
            return maybe_struct!((Some( msg),Some(Utc::now().timestamp())));
        }
        Err(msg) => {
