...
pub async fn query_api_anc_ust_lp_reward() ->  anyhow::Result<ResponseResult> {
    ...
    let res: String = query_api("https://api.anchorprotocol.com/api/v2/ust-lp-reward").await?;
...
pub async fn query_api_spec_anc_ust_lp_reward() ->  anyhow::Result<ResponseResult> {
    ...
    let res: String = query_api("https://specapi.azurefd.net/api/data?type=lpVault").await?;
