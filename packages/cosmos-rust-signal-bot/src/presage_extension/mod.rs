...
use crate::cosmos_rust_bot::{process_message, handle_notifications};
...
pub async fn process_message_from_self<C: ConfigStore>(sender_uuid: Uuid, sender_message: String, manager: &Manager<C, Registered>) {
    ...
    while !msg_sent {
        let text = process_message(sender_message.to_owned()).await;
...
pub async fn run_cosmos_rust_signal_bot<C: ConfigStore>(manager: Manager<C, Registered>) {
