...
pub async fn anchor_redeem_and_repay_stable(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, wallet_acc_address: Arc<SecUtf8>, wallet_seed_phrase: Arc<SecUtf8>, only_estimate: bool) -> String {
    let max_tx_fee = decimal_or_return_str!(max_tx_fee_to_string(tasks.clone(), 4).await.as_ref(),"(waiting for max_tx_fee)");
	let balance = decimal_or_return_str!(terra_balance_to_string(tasks.clone(),"uusd",false,2).await.as_ref(),"(waiting for ust_balance)");
 	let min_ust_balance = decimal_or_return_str!(min_ust_balance_to_string(tasks.clone(),false,2).await.as_ref(),"waiting for min_ust_balance)");
    ...
    if to_withdraw_from_deposit > zero && to_repay > zero {
		...
		let gas_adjustment_preference = decimal_or_return_str!(gas_adjustment_preference_to_string(tasks.clone(),10).await.as_ref(),"(waiting for gas_adjustment_preference)");
