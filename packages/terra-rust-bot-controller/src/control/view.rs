...
pub async fn calculate_borrow_plan(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field: &str, digits_rounded_to: u32) -> String {
    ...
    let min_ust_balance = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"min_ust_balance",false,10).await.as_ref());
...
pub async fn calculate_repay_plan(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field: &str, digits_rounded_to: u32) -> String {
    ...
    let ust_balance_preference = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"ust_balance_preference",false,10).await.as_ref());
...
pub async fn calculate_amount(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, key: &str, as_micro: bool, digits_rounded_to: u32) -> String {
    ...
    let target_percent = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"target_percentage",false,10).await.as_ref()); 
    let zero =  Decimal::from_str("0").unwrap(); 
 
    //let trigger_percentage = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await.as_ref());
...
pub async fn check_anchor_loan_status(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>,key: &str, digits_rounded_to: u32) -> String {
    ...
    let trigger_percentage = match key {
        "repay" => {decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await.as_ref())},
        "borrow" => {decimal_or_return!(meta_data_key_to_string(tasks.clone(),"borrow_percentage",false,10).await.as_ref())},
...
pub async fn estimate_anchor_protocol_next_claim_and_stake_tx(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, field_type: &str, field_amount: &str, field: &str, digits_rounded_to: u32) -> String {
            ...
            if "loan_amount"==field_amount {
                loan_amount = decimal_or_return!(borrower_loan_amount_to_string(tasks.clone(),false, 10).await.as_ref()); 
            }else if "target_ltv"==field_amount {
                let trigger_percentage = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await.as_ref());
                let borrow_limit = decimal_or_return!(borrow_limit_to_string(tasks.clone(), false, 10).await.as_ref()); 
...
pub async fn apy_on_collateral_by(tasks: Arc<RwLock<HashMap<String, MaybeOrPromise>>>, amount_field: &str, apr_field: &str, digits_rounded_to: u32) -> String { 
    ...
    if amount_field == "loan_amount" {
        ...
        loan_amount = decimal_or_return!(meta_data_key_to_string(tasks.clone(),"trigger_percentage",false,10).await.as_ref())
